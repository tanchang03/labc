...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    mov   rax , 0
  newcs:
    add   rax , [pr+0x20000+rcx]
    add   rcx , 8
    cmp   rcx , scalex*scaley*3
    jb    newcs
...
RUNNINGAPPS         equ 1024 + BMP_BC + BMPHEADER + (BPP * BMP_BC_DEPTH * (BMP_BC_WIDTH+3))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  noup:
    ...
    jmp     changepos
  nodown:
    jmp     noipc ; unknown ipc message, like late image send
  changepos:
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  newcheck:
    ...
    xor     rax , rax
  newmp:
    cmp     [0xf0000+408+rax+1],dword 'CALR'
    jne     nomp2
    cmp     [0xf0000+408+rax+0],byte '/'
    je      still
    cmp     [0xf0000+408+rax+0],byte '\'
    je      still
  nomp2:
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  ran003:
    ...
    ;
    ; 3) Arrange according to PID (start time)
    ;
    mov    r8  , 0
  arrangepids:
    mov    rax , [labelcount]
  ...
  noarrange:
    ...
    mov    r13 , app_ascii_utf8  ; ascii/utf8
  ...
  scan001:
    ...
    int    0x60
    ;
    mov    r15 , [OSWORKAREA+32]
    shr    r15 , 60
    and    r15 , 1
    mov    [r13],r15b
    pop    r15 r8 rdx rcx rbx rax
  ...
  ranexit:    
    ...
    ;
    ; 5) Do we have more windows we can display ?
    ;
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    ; Decode included image

    call    decode_external

    ; Separate images

    mov     rsi , image_decode
    mov     rdi , BMP_1+BMPHEADER
    mov     rcx , BMP_1_WIDTH*BMP_1_DEPTH*3
    cld
    rep     movsb

    mov     rsi , image_decode+45*140*3
    mov     rdi , BMP_SCC+BMPHEADER
    mov     r15 , 0
  newscch:
    push    rsi
    mov     rcx , BMP_SCC_WIDTH*3
    cld
    rep     movsb
    pop     rsi
    add     rsi , 140*3
    add     r15 , 1
    cmp     r15 , BMP_SCC_DEPTH
    jb      newscch

    mov     rsi , image_decode+45*140*3+10*3
    mov     rdi , BMP_SCL+BMPHEADER
    mov     r15 , 0
  newsclh:
    push    rsi
    mov     rcx , BMP_SCL_WIDTH*3
    cld
    rep     movsb
    pop     rsi
    add     rsi , 140*3
    add     r15 , 1
    cmp     r15 , BMP_SCL_DEPTH
    jb      newsclh

    mov     rsi , image_decode+45*140*3+30*3
    mov     rdi , BMP_SCR+BMPHEADER
    mov     r15 , 0
  newscrh:
    push    rsi
    mov     rcx , BMP_SCR_WIDTH*3
    cld
    rep     movsb
    pop     rsi
    add     rsi , 140*3
    add     r15 , 1
    cmp     r15 , BMP_SCR_DEPTH
    jb      newscrh

    mov     rsi , image_decode+45*140*3+50*3
    mov     rdi , BMP_BC+BMPHEADER
    mov     r15 , 0
  newbch:
    push    rsi
    mov     rcx , BMP_BC_WIDTH*3
    cld
    rep     movsb
    pop     rsi
    add     rsi , 140*3
    add     r15 , 1
    cmp     r15 , BMP_BC_DEPTH
    jb      newbch
...
decode_external:
    ...
    mov   rax , 0
    mov   [image_decode-32],rax
    mov   rax , 16
    mov   [image_decode-24],rax
    mov   rdi , image_decode
    mov   rcx , 0x080000
    mov   rax , 0xb0
    cld
    rep   stosb
    mov   [image_decode],dword 123123

    ; Define IPC

    mov   rax , 60
    mov   rbx , 1
    mov   rcx , image_decode-32
    mov   rdx , 0x080000
    int   0x60
  ...
  waitmore:
    ...
    inc   rdi
    cmp   rdi , 1000*5 ; 5 second timeout
    ja    notransformation
...
imanager:    db  '\sysdir\imanager',0
string_boot: db  'BOOT',0 
runimanager: dq  0x0 
prev_start:  dq  0x0
string_cpu:  db  '\sysdir\CAD',0
...
rundecoder:     db  '\sysdir\GIFVIEW',0
param:          db  'PARAM',0
...
start_boot:   db  '\sysdir\launcher',0
...
calendar:    db  '\sysdir\CALR',0
startfile2:  db  '\sysdir\MMENU',0
paramsup:    db  'MAINMENU 5 28',0
paramsdown:  db  'MAINMENU 04 0258',0

clearstart:

app_ascii_utf8: times 300 db ?
